...
`scroll/silver_session_recent.sql` shows a rolling "last N days" window kept incrementally refreshable by using a fixed date parameter (`window_start_date`, advanced per run) rather than `current_date()`.
...
Set your CLI profile in `databricks.yml` (`targets.dev.workspace.profile`) and your `catalog`/`schema` via the bundle variables (defaults: `main` / `sc_fs_demo`).
...
To target a different catalog/schema without editing files: `databricks bundle deploy --var catalog=my_catalog --var schema=my_schema`.
